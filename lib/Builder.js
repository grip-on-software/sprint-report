...
class Builder {
    ...
    formatDate(data, node, key) {
        const date = this.localization.moment(data, "YYYY-MM-DD HH:mm:ss", true);
    ...
    updateFeatures(keys) {
        features = _.intersection(_.map(keys, feature =>
        ...
        const updateProjects = (element, list) => {
            ...
            element.attr('class', d => `tooltip is-tooltip-multiline is-tooltip-center ${d.classes || ""}`)
                .attr('data-tooltip', d => {
                    ...
                    const msg = `${prefix}-title-${isActive(d.name) ? "remove" : "add"}`;
                    return this.locales.message(msg, [d.quality_display_name]);
                })
                .attr('href', d => {
                    var projects = null;
                    if (d.projects) {
                        projects = d.projects(list.merge(list.enter()));
                    }
                    else {
                        projects = setToggle(OrderedSet(this.projects.selected),
                            d.name,
                            _.isArray(d.project_names) ? d.project_names : null
                        );
                    }
    ...
    makeSprintSelection() {
        ...
        const old = d3.select('#sprints-old')
    ...
    makeSourceAge() {
        ...
        axios.all(_.map(projects,
            ...
            newAge.append('span')
                .attr('title', d => d.date)
                .text(d => this.localization.moment(d.date, "YYYY-MM-DD HH:mm:ss", true).format('ll'));
            newAge.append('a')
