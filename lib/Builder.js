...
class Builder {
    ...
    makeSprintSelection() {
        ...
        this.makeSprintFilter();
        this.makeSprintMeta();
    }

    makeSprintBrush() {
        const max = (name, default_value) => {
            const meta = _.maxBy(_.filter(this.projects.meta,
        ...
        const num_sprints = max('num_sprints', this.sprints.limit);
        const future_sprints = this.formatter.current !== null &&
            this.sprints.future > 0 &&
            this.formatter.current.requestConfig().future &&
            !this.features.selected.intersect(this.features.future).isEmpty() ?
            max('future_sprints', 0) : 0;

        this.sprints.current = Math.max(0,
            Math.min(this.sprints.current, num_sprints)
        );
        this.sprints.first = Math.max(-future_sprints,
            Math.min(this.sprints.first, this.sprints.current)
        );

        var tooltip;
        if (this.sprints.first < 0) {
            tooltip = 'sprints-count-tooltip-future';
        }
        else {
            tooltip = `sprints-count-tooltip${this.sprints.first === 0 ? "-recent" : ""}`;
        }
