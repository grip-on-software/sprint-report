...
class Builder {
    ...
    makeFeatureSelection() {
        ...
        const tabs = newCategories.merge(categories)
            .selectAll('.features .tabs ul');
        ...
        const updateFeatures = newFeatures.merge(features)
            .classed('is-hidden', d => !this.features.selected.has(d) &&
                !_.some(formatFeatures, group => this.features[group].has(d))
            )
        ...
        d3.select(newCategories.node())
            ...
            .classed('tabs is-toggle is-size-7', true)
            .append('ul');
        const selection = categories.select('.selection ul')
            .selectAll('li')
            .data([{
                features: () => OrderedSet(),
                message: 'clear',
