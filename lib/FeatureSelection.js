...
export default class FeatureSelection {
    ...
    toggleFeatureSelection(selection, panel, visible=null, duration=500) {
        ...
        const features = d3.select('#features .columns');
    ...
    makeSelectedFeatures() {
        ...
        if (!this.state.features.changed) {
            this.toggleFeatureSelection(selection, panel, false, 0);
        }
        ...
        newIcon.merge(icons)
            ...
            .attr('href', (d, i) => {
                var features = _.assign({}, this.state.features);
                if (d.options) {
