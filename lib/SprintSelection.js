...
export default class SprintSelection {
    ...
    makeSprintSelect() {
        const buttons = [
            {
                ...
                url: state => getUrl(state, {
                    count: [0, state.sprints.limit]
                })
            },
            {
                id: 'sprints-minus-one',
                icon: 'fas fa-minus',
                active: state => state.sprints.current > 0,
                url: state => getUrl(state, {
                    count: [state.sprints.first, state.sprints.current - 1]
                })
            },
            {
                id: 'sprints-plus-one',
                icon: 'fas fa-plus',
                active: state => state.sprints.current < this.max('num_sprints', state.sprints.limit),
                url: state => getUrl(state, {
                    count: [state.sprints.first, state.sprints.current + 1]
                })
            },
            {
                ...
                url: state => getUrl(state, {
                    count: [
                        Math.min(0, state.sprints.first),
        ...
        const newButtons = button.enter()
            ...
            .attr('id', d => d.id)
            .attr('data-tooltip', d => this.locales.message(d.id))
            .attr('aria-label', d => this.locales.message(d.id));
