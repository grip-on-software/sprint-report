...
export default class SprintSelection {
    ...
    makeSprintTicks(x, num_sprints, future_sprints, transform) {
        ...
        const tick_distance = Math.abs(x.invert(90) - x.invert(0));
        const future_distance = Math.abs(x.invert(110) - x.invert(0));
        const ends = transform([
            ticks => [0, 1],
            ticks => [ticks.length - 1, -1]
        ]);
        const [future, future_dir] = ends[0](ticks);
        var future_count = future_dir;
        while (ticks[future + future_count] < 0 &&
            Math.abs(ticks[future + future_count] + future_sprints) < future_distance
        ) {
            future_count += future_dir;
        }
        ticks.splice(Math.min(future, future + future_count - future_dir),
            Math.abs(future_count), -future_sprints
        );

        const [past, past_dir] = ends[1](ticks);
        var past_count = past_dir;
        while (Math.abs(num_sprints - ticks[past + past_count]) < tick_distance) {
            past_count += past_dir;
        }
        ticks.splice(Math.min(past, past + past_count - past_dir),
            Math.abs(past_count), num_sprints
        );
        return ticks;
    }
    ...
    makeSprintBrush() {
        ...
        const domain = transform([-future_sprints, num_sprints]);
