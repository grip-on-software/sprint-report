    ...
    makeSprintBrush() {
        ...
        const brush = d3.brushX()
            ...
            .on("end", () => {
                ...
                var pos = d3.event.selection.map(x.invert).map(Math.round);
                ...
                window.location = getUrl(this.state, {
                    count: pos[0] >= 0 ? [first, pos[0], pos[1]] :
                        [pos[0], 0, pos[1]]
                });
            });
        const disjointBrush = d3.brushX()
            .extent([[0, 0], [width, height + 16]])
            .filter(() => {
            ...
            .on("end", () => {
                ...
                var pos = d3.event.selection.map(x.invert).map(Math.round);
                window.location = getUrl(this.state, {
                    count: [pos[0], pos[1] > current ? 0 : current, last]
                });
            });
