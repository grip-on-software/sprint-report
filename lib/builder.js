...
class builder {
    constructor(projects, features, locales, moment, localization, sprints) {
        this.projects = {
    ...
    makeSprintSelection() {
        ...
        const hasClosed = this.sprints.closed ? null : true;
        const closed = d3.select('#sprints-closed label')
            .attr('disabled', hasClosed)
            .select('input')
            .attr('disabled', hasClosed)
            .attr('checked', hasClosed)
            .on('change.close', () => {
                window.location = this.getUrl({
    ...
    filterSprints(sprints) {
        return _.take(this.sprints.closedOnly ?
            _.filter(sprints, d => d.sprint_is_closed) : sprints,
            this.sprints.current
        );
    }
