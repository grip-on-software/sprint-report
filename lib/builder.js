...
class builder {
    ...
    makeProjectNavigation() {
        ...
        const updateProjects = (element) => {
            ...
            element.text(d => d.name === "" ?
                ...
                d.name === "*" ? this.locales.message("projects-select-all") :
                d.name)
                .attr('href', d => {
                    return this.getUrl({project: (d.name === "" ? OrderedSet() :
                        d.name === "*" ? OrderedSet(this.projects.known) :
    ...
    makeTable() {
        ...
        const auxiliaryKeys = _.concat("",
            ...
            ), this.sprints.showOld ? "old" : [], "links"
        );
        this.spinner.start();
        axios.all(_.flatten(_.map(selectedProjects, project => {
            ...
            const data = _.map(projectRequests, (project) => {
                return {
                    ...
                    "meta": Array.from(this.sprint_meta.selected)
                };
            });
                ...
                newSprint.merge(sprintHeader).each((d, i, nodes) => {
                    const meta = d3.select(nodes[i]).select('.tags')
                        ...
                        .data(d => _.toPairs(_.pick(d,
                            Array.from(this.sprint_meta.selected)
                        )));
                    meta.exit().remove();
