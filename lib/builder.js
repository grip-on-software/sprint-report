...
class builder {
    constructor(projects, features, expressions, locales, moment, localization, sprints, spinner) {
        this.projects = {
    ...
    makeProjectNavigation() {
        ...
        const updateProjects = (element, list) => {
            ...
            element.attr('data-tooltip', d => d.title ||
    ...
    makeFeatureSelection() {
        ...
        label.each((d, i, nodes) => {
            ...
            const assignment = this.getAssignment(this.features.expressions[d]);
    ...
    getAssignment(assignment) {
        ...
        if (assignment.attributes) {
            const features = this.localization.descriptions;
            return _.replace(assignment.expression,
                new RegExp(`(^|\\W)(${_.join(assignment.attributes, '|')})(\\W|$)`, "g"),
                (m, p1, attribute, p2) => `${p1 || ""}${this.locales.retrieve(features, attribute)}${p2 || ""}`
            );
        }
    ...
    makeFormat() {
        ...
        }, this.spinner).then(() => this.setTitle());
    }
