...
const findTabs = (container, viewX, viewY) => {
    const view = d3.mouse(document.body);
    if (typeof document.elementsFromPoint === "function") {
        return d3.selectAll(document.elementsFromPoint(view[0], view[1]))
            .filter('.tabs a:not(.dragging)');
    }
    return d3.select(container).selectAll('.a:not(.dragging)').filter(function() {
        const rect = this.getBoundingClientRect();
        return rect.left <= view[0] && view[0] <= rect.left + rect.width &&
            rect.top <= view[1] && view[1] <= rect.top + rect.height;
    });
};
...
class builder {
    ...
    makeProjectNavigation() {
        ...
        const updateProjects = (element, list) => {
            ...
            element.select('span.tag')
                .classed("is-hidden", d => !order[d.name])
                .text(d => order[d.name] || null);
        };
                ...
                element.call(d3.drag()
                    ...
                    .on("start", (d, i, nodes) => {
                        ...
                        .on("end", (d, i, nodes) => {
                            ...
                            const t = dropTab.datum();
                            if (!isActive(d.name) || !isActive(t.name)) {
                                return;
                            }
                            window.location = this.getUrl({
                                project: _.map(Array.from(this.projects.selected),
                                    name => name === d.name ? t.name :
                                        name === t.name ? d.name : name
                                )
                            });
                        });
                    })
