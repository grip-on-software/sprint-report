...
class builder {
    ...
    makeToggle() {
        ...
        d3.selectAll('#options .toggle')
            ...
            .on('click', (d, i, nodes) => {
                ...
                d3.select(nodes[i])
                    .attr('title', this.locales.message(`${d.toggle}-${hidden ? "hide" : "show"}`))
                    .select('i')
            ...
            .on("start", (d, i, nodes) => {
                ...
                .on("end", (d, i, nodes) => {
                    ...
                    d3.select(nodes[i])
                        ...
                        .style("top", "0px");
                    if (dropTab.empty()) {
                        console.log("cannot find drop tab");
                        return;
                    }
                    ...
                    if (!config.items.selected.has(drag) ||
                        ...
                        console.log("drop tab is not selected: " + drop);
                        return;
                    }
    ...
    makeExportOptions() {
        ...
        button.append('span')
            .classed('icon', true)
            .append('i')
            .classed('fas fa-th', true);
        button.append('span')
            .text('CSV');
        button.on('click', () => {
            const exporter = new exports.CSV({
                projects: this.projects,
                features: this.features,
                sprints: this.sprints,
                sprint_meta: this.sprint_meta
