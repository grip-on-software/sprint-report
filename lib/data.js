...
const updateFutureFeature = function(feature, futureSprints, latestSprint,
    prediction, previous, key
) {
    return _.mapValues(feature, (predict, subkey) => {
        ...
        return Math.max(0, _.isObject(previous) ? previous[subkey] - step :
            previous - step
        );
    });
};
...
const updateFutureSprints = function(sprints, futureSprints, latestSprint,
    ...
    const downtime = length + futureSprints.length < 2 ? 0 :
        moment(futureSprints[1].start_date).diff(futureSprints[0].close_date);
    return _.transform(sprints, (res, sprint, index) => {
        if (sprint.future) {
            sprint = _.mapValues(sprint, (feature, key) => {
                if (key === "start_date" || key === "close_date") {
                    return moment(res[index-1][key]).add(downtime, 's')
                        .format('YYYY-MM-DD HH:mm:ss');
                }
                else if (index > 0 && _.isObject(feature) &&
