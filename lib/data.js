...
const sprintsToFeatures = function(sprints, features, visible=null, config={}, metric_targets={}) {
    ...
    return _.reduce(Array.from(features), (accumulator, feature, index) => {
        accumulator.push(sprintsToFeature(sprints, accumulator, {
            index,
            feature_key: feature,
            metric_targets: metric_targets ? metric_targets[feature] : []
        }, visible, config));
        return accumulator;
    }, []);
};
...
class FutureSprints {
    ...
    updateFeature(feature, previous, sprint_num, key, sprint) {
        return _.transform(feature, (res, predict, subkey) => {
            ...
            if (end) {
                this.state.sprint_meta.selected.forEach(meta => {
                    const error_key = `${subkey}-${meta}_error`;
                    if (meta === "sprint_num") {
                        res[error_key] = end[meta] - sprint[meta];
                    }
                    else if (meta !== "sprint_name") {
                        res[error_key] = moment.duration(moment(end[meta])
    ...
    update(sprints) {
        const start_date = moment(this.latestSprint.start_date);
        const close_date = moment(this.latestSprint.close_date);
...
const makeRequests = function(state, config) {
    ...
    return new Promise((resolve, reject) => {
        requests.then((responses) => {
            const projectRequests = _.zip(Array.from(state.projects.visible),
