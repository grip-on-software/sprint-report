...
const zipFeature = function(featureSprints) {
    ...
    return _.zip(..._.map(featureSprints, g => _.isObject(g) ?
        _.filter(g, (v, k) => !k.endsWith("_error")) :
...
const updateFutureFeature = function(feature, futureSprints, latestSprint,
    prediction, previous, key, sprintData, sprint
) {
    return _.transform(feature, (res, predict, subkey) => {
        const step = prediction && prediction[subkey] &&
            prediction[subkey].reference ?
            latestSprint[prediction[subkey].reference] :
            latestSprint[key] - futureSprints[0][key][subkey];
        res[subkey] = Math.max(0, _.isObject(previous) ?
            previous[subkey] - step : previous - step
        );
        const end = _.find(sprintData, d => {
            const f = _.isObject(d[key]) && d[key][subkey] ? d[key][subkey] :
                d[key];
            return d.sprint_num >= latestSprint.sprint_num &&
                d.sprint_num <= futureSprints[0].sprint_num && f <= res[subkey];
        });
        res[`${subkey}_error`] = end ? end.sprint_num - sprint.sprint_num : 0;
    }, {});
};

const updateFutureSprints = function(sprints, futureSprints, latestSprint,
    sprintData, startIndex, prediction
) {
    const start_date = moment(latestSprint.start_date);
    const close_date = moment(latestSprint.close_date);
    const length = close_date.diff(start_date);
    const downtime = length +
        (futureSprints.length < 2 ? 0 : moment(futureSprints[1].start_date)
            .diff(futureSprints[0].close_date));
        ...
        if (sprint.future && index > 0) {
            sprint = _.mapValues(sprint, (feature, key) => {
                if (key === "start_date" || key === "close_date") {
                    ...
                    return updateFutureFeature(feature, futureSprints,
                        latestSprint, prediction, res[index-1][key], key,
                        sprintData, sprint
                    );
                }
                return feature;
    ...
    return new Promise((resolve, reject) => {
        requests.then((responses) => {
            ...
            const projects = _.map(projectRequests, (project) => {
                ...
                const sprintData = requestsToSprints(data, keys);
                var {sprints, futureSprints, latestSprint} =
                    filterSprints(state, sprintData);
                if (state.sprints.current > 0 && latestSprint !== null) {
                    sprints = updateFutureSprints(sprints, futureSprints,
                        latestSprint, sprintData,
                        sprintData.length - futureSprints.length -
                        state.sprints.last, config.metadata.prediction
                    );
                }
