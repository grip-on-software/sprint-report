...
class HTML extends Export {
    ...
    writeStatic(resolve, reject) {
        ...
        axios.all(staticRequests).then((staticResponses) => {
            ...
            data.file('features.json', this.json(_.omit(_.mapValues(this.state.features,
...
class PDF extends Export {
    build(button) {
        const titlePart = Array.from(this.state.projects.selected).join(', ');
        ...
        const url = vsprintf(config.render_url, [
            ...
            titlePart === '' ? this.locales.message("title") :
                this.locales.message("print-title", [titlePart])
        ]);
