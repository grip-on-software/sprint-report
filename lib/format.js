...
class Chart extends Format {
    ...
    updateFocus(chart, data, state, sprints, y, callbacks) {
        ...
        const selectFeature = (eventTarget) => {
            ...
            console.log({target, i, j, feature, index});
        ...
        const showTooltip = (index, i, feature) => {
            ...
            const meta = text.selectAll("tspan.meta")
                .data(_.toPairs(_.pick(sprints[index], callbacks.filter(
            ...
            const newOptions = options.enter().append("g")
                ...
                .each((d, i, nodes) => {
                    ...
                    const optionText = text.append("tspan")
                        ...
                        .text(`\u00A0${d.text || ""}`);
                    const bbox = optionText.node().getBoundingClientRect();
                    const cbox = tooltip.node().getBoundingClientRect();
                    const option = d3.select(nodes[i]);
                    option.attr("transform", `translate(${bbox.left - width - cbox.x}, ${bbox.top - cbox.y})`);
                    option.append("rect")
                        .attr("width", bbox.width + width + padding)
        ...
        this.pinTooltip = (eventTarget) => {
            ...
            if (fixed) {
                if (target[0] >= pos[0] &&
                    ...
                    else if (meta.classed("meta")) {
                        const link = callbacks.link(meta.datum(), current);
                        if (link) {
            ...
            updateFocusPosition(target, index, fixed ? [i, feature] : [0]);
            showTooltip(current, i, feature);
        };
        ...
        chart.select('.overlay')
            ...
            .on("mousemove.tooltip", () => {
                ...
                const { target, i, feature, index } = selectFeature(d3.event.currentTarget);
