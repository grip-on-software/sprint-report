...
class Table extends Format {
    ...
    format(data, state) {
        ...
        updateProjects.each((d, i, nodes) => {
            this.addSourceIcon(d3.select(nodes[i]).selectAll('td.name'), d);
        });
        ...
        item.append('a')
            ...
            .on('click', (d, i, nodes) => {
                ...
                const table = sprints.append('table')
                    ...
                    .datum((s, j) => {
                        const sprint = data.details[d.feature_key][data.sprints[j].sprint_id];
...
class Chart extends Format {
    ...
    updateLegends(state, updateProjects) {
        ...
        const legend = legends.enter()
            .append("g")
            .classed("line", true)
            .attr("transform", (d, i) => `translate(0,${10 + i * this.textHeight})`);
        ...
        legendHolder.each((d, i, nodes) => {
            ...
            d3.select(nodes[i]).select("rect.box")
                ...
                .attr("height", state.features.selected.size === 1 ? 0 :
                    10 + (state.features.selected.size + 1) * this.textHeight
                );
        });
    }
    ...
    updateFocus(chart, state, sprints, y, callbacks) {
        ...
        chart.select('.overlay')
            ...
            .on("mousemove", () => {
                ...
                const feature = callbacks.select(i, j, target);
                ...
                const meta = text.selectAll("tspan")
                    .data(_.toPairs(_.pick(sprints[i],
    ...
    updateAxes(chart, state, d, x, y) {
        ...
        chart.select('.label.x')
            .text(this.locales.attribute("sprint_meta",
                getSprintMeta(state.sprint_meta, "main")
            ));
    }
}
...
export default { Table, LineChart, BarChart };
