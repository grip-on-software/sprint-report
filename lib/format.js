...
class ScatterPlot extends Chart {
    initialize() {
        ...
        this.brush = d3.brush()
            .on("start", () => {
                focus.select(".details").classed("is-hidden", true);
    ...
    pointInDomain(point) {
        return (
            d3.bisectLeft(this.x.scale.domain(), point[this.x.feature]) === 1 &&
            d3.bisectLeft(this.y.scale.domain(), point[this.y.feature]) === 1
        );
    }
