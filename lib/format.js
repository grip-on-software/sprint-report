...
class Chart extends Format {
    ...
    createCharts(data) {
        const projects = this.content.selectAll('svg')
            .data(data, d => d.project_name);
        projects.exit().remove();
        var { newCharts, newProjects, newLegendHolder } =
    ...
    buildChartDimensions(content) {
        ...
        return { newCharts, newProjects, newLegendHolder };
    }
...
class ScatterPlot extends Chart {
    ...
    format(data, state) {
        ...
        const points = _.flatten(_.map(data,
            (project, i) => _.map(project.sprints,
                sprint => _.assign({}, _.pick(sprint, _.concat(features,
        ...
        this.updateFocus(chart, state, points, y, {
            ...
            select: (i, j) => {
                ...
                const minimalPoint = _.reduce(points,
                    (minimal, point, index) => {
                        const distance = Math.abs(point[features[0]] - i) +
                            Math.abs(point[features[1]] - j);
