...
export default class AreaChart extends Chart {
    ...
    format(data, state, resolve) {
        ...
        const stackOnly = _.filter(select, (f, i) => !stackIndexes.has(i));
        this.stackFeatures = _.map(stackOnly, f => _.isArray(f) ? f[0] : f);
        this.stackKeys = _.map(stackOnly,
            f => _.isArray(f) ? `${select[f[1]]}-${f[0]}` : `${f}-${f}`
        );
    ...
    updateChart(state, chart, d, i, t) {
        ...
        const stack = d3.stack()
            .keys(this.stackFeatures)
            .value((d, key) => unwrap(d[key]));
        const stackValues = stack(d.sprints);
        const stacks = _.zipObject(this.stackKeys, stackValues);
