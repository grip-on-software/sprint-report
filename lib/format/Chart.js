...
import {unwrapFeature, filterSprints, sprintsToFeatures, getSprintMeta} from '../data';
...
export default class Chart extends Format {
    ...
    createFeatures(state, updateProjects, config={}) {
        ...
        if (config.stacks) {
            select = _.reduce(select, (accumulator, feature) => {
        ...
        newFeatures.merge(features)
            .style("visibility", d =>
                state.features.visible.has(d.feature_key) || d.stack !== null ?
                null : "hidden"
            );
    ...
    updateFocus(chart, data, state, sprints, x, y, callbacks) {
        callbacks = _.assign({}, {
            ...
            highlight: (d, i, feature) => d[1] === feature ||
                (d[1] !== undefined && d[1] !== null && d[1].max === feature),
        ...
        const showTooltip = (index, i, feature, pos) => {
            ...
            const metadata = _.toPairs(_.pick(sprints[index],
                callbacks.filter(_.concat(
            ...
            meta.enter().append("tspan")
                ...
                .classed("has-source",
                    (d, i, nodes) => callbacks.has_source(d, i) &&
                        callbacks.link(d, index, d3.select(nodes[i]), -1).type
                )
                .classed("highlight",
                    d => callbacks.highlight(d, index, feature)
                )
                .each((d, j, nodes) => {
                    const node = d3.select(nodes[j]);
                    const source = callbacks.link(d, index, node, -1).type;
                    const hasSourceIcon = source && this.localization.sources &&
                    ...
                    var text = null;
                    if (state.sprint_meta.selected.includes(d[0])) {
                        text = this.formatSprint(d, nodes[j],
                            state.sprint_meta
                        );
                    }
                    else if (state.features.selected.includes(d[0])) {
                        const description = node.append('tspan')
                            .text(`${this.locales.retrieve(this.localization.descriptions, d[0])}: `);
                        this.formatFeature(d[0], d[1], node.append('tspan'),
                            tooltip, {
                                class: d[0],
                                scale: 0.025,
                                top: this.tooltipPadding.top +
                                    offset.height * j + 4,
                                left: this.tooltipPadding.left +
                                    description.node().getComputedTextLength() +
                                    4 + (hasSourceIcon ? offset.icon : 0)
                            }
                        );
                    }
                    else {
                        text = callbacks.format(d[0], d[1], nodes[j]);
                    }
                    if (text !== null) {
                        node.text(text);
                    }
            ...
            if (fixed) {
                if (target[0] >= position[0] &&
                    ...
                    const item = tooltip.selectAll("tspan.meta, tspan.option")
                        .filter((d, j, nodes) => {
                            ...
                            if (!m.classed("has-source") &&
                                m.select("tspan.has-source").empty()
                            ) {
                                return false;
                            }
                            var rect;
                    ...
                    if (item.classed("option")) {
                        ...
                        const link = callbacks.link(item.datum(), current, item,
                            target[0] - position[0] - this.tooltipPadding.left
                        );
                        if (link) {
                            const source = d3.select(document.body)
                                ...
                                .attr('target', '_blank')
                                .attr('href',
                                    callbacks.makeLink(link.source, current)
                                );
                            source.node().click();
                            source.remove();
                            return true;
            ...
            x: {
                label: this.locales.attribute("sprint_meta",
                    getSprintMeta(state.sprint_meta,
                        state.sprint_meta.changed ? "main" : "sprint_num"
                    )
                ),
