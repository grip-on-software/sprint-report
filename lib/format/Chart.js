...
export default class Chart extends Format {
    ...
    updateLegends(state, updateProjects, config) {
        ...
        extraLine.attr("fill-opacity", 0)
            .merge(extra).order()
            .transition()
            .duration(1000)
            .attr("fill-opacity", d => config.extraVisible(d) ? 1 : 0.33);
    ...
    clickLegendRow(state, row) {
        if (row >= 0) {
            ...
            line.transition(t)
                .attr("fill-opacity", visible ? 1 : 0.33);
    ...
    updateAxes(chart, state, d, x, y, config={}) {
        ...
        _.forEach(['x', 'y', 'x2', 'y2', 'y0', 'x0', 'yx', 'future'], name => {
            if (config[name] !== null) {
                ...
                axis.update(config.t, x, y, this.locales, this.localization);
            }
        });
    }
