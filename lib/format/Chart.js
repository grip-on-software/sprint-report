...
export default class Chart extends Format {
    ...
    resize() {
        const width = this.content.node().clientWidth - 64;
        this.svgWidth = Math.max(960, width);
        this.svgHeight = Math.round(width / 2.5);
    ...
    createCharts(data, axes={}) {
        ...
        var { newCharts, newProjects, newLegends } =
            this.buildChartDimensions(projects.enter(), axes);
        newLegends.append("g")
            .classed("header", true)
