    ...
    updateAxes(chart, state, d, x, y, config={}) {
        ...
        if (config.yx !== null) {
            chart.select('.axis.yx')
                .classed("is-invisible", !config.yx)
                .transition(config.t)
                .select("line")
                .attr("x1", Math.max(0, x(y.domain()[0]) || 0))
                .attr("x2", Math.min(width, x(y.invert(0)) || 0))
                .attr("y1", Math.min(this.height, y(x.domain()[0]) || 0))
                .attr("y2", Math.max(0, y(x.invert(width)) || 0));
        }
    }
