...
/* Generate evenly spaced numbers within an interval [start, stop] */
...
export default class Chart extends Format {
    initialize() {
        ...
        // Patterns that can be used as repeating glyphs that differentiate
        // features and indicate feature values at sprint dates, for example
        // on a line.
    ...
    /**
     * Retrieve the SVG reference to a glyph pattern based on the feature index.
     */
    patternRef(i) {
    ...
    /**
     * Adjust dimensions of the chart based on the screen size. Only the width
     * and height variables are changed, not the actual rendered element.
     */
    resize() {
    ...
    /**
     * Create the chart report. One or more chart elements are created, which
     * depends on the format type (some formats combine projects together in
     * one chart).
     */
    createCharts(data, state, axes={}) {
        // Add pattern definitions to the report
    ...
    /**
     * Add an event listener that resizes the chart(s) when the screen is
     * resized. This only happens when the browser has time left to render
     * changes, to avoid spamming with changes during a manual drag resize.
     */
    ...
    /**
     * Create or update the chart with initial dimensions, axes and legends
     * which are to be filled and updated afterward.
     */
    buildChartDimensions(content, axes={}, t=null) {
        const width = axes.y2 ? this.width - this.margin.yAxis : this.width;
        const newCharts = content.enter().append('svg')
            .classed('chart', true);
        ...
        const newProjects = newCharts.append("g")
        ...
        // Diagonal line through origin dividing the chart in half y<x and y>x
        if (axes.yx) {
            newProjects.append("g")
    ...
    /**
     * Create a transition that is used by other create/update functions in
     * order to synchronize animations, and once the transition end the promise
     * is resolved.
     */
    ...
    /**
     * Create a focus element which may be used to put focus on sprint points
     * when the user hovers over the newly made charts, and displays a tooltip.
     * The empty elements do not provide any interaction yet until actual focus
     * elements are created in the holder element, for example the circle and
     * line from `Chart.createLineFocus`, and when `Chart.updateFocus` and
     * possibly `Chart.updateLineFocus` (via the `focus` option) is called.
     */
    ...
    /**
     * Create a focus element which can be used to put focus on sprint points
     * when the user hovers over the newly made charts, and displays a tooltip.
     * The focus element uses a circle to put the focus and a line to connect it
     * to an axis. The elements do not provide any interaction yet until we call
     * `Chart.updateFocus` and `Chart.updateLineFocus` (via the `focus` option).
     */
    ...
    /**
     * Update the circle and line using the the focus element, based on the
     * sprint that was focused on and the positions leading from it.
     */
    ...
    /**
     * Expand an array of selected features to include the attributes used for a
     * compound feature which had an expression with only simple additions.
     * The attributes are added after the feature as a pair with the attribute
     * key as the first element and the index of the feature it was used in as
     * the second element.
     */
    ...
    /**
     * Create or update SVG grouping elements for features with sprint data
     * bound to the features. The newly created feature elements are empty,
     * `Chart.updateFeatures` is meant to fill them with selected data.
     */
    createFeatures(state, updateProjects, config={}) {
        let select = Array.from(state.features.selected);
        if (config.stacks) {
    ...
    /**
     * Fill in the legends of the chart(s) to indicate what data is in them,
     * using samples and legend text, possibly with extra lines.
     */
    updateLegends(state, updateProjects, config) {
        ...
        // Update the legends with the selected data (remove old and add new)
        legends.exit().remove();
        ...
        // Create samples and text next to them
        ...
        // Track the dimensions of each legend after being filled and adjust
        // the SVG element size to fit it (synchronized with the transition)
        this.legendWidths = [];
        this.legendHeights = [];
    ...
    /**
     * Fill in the legends of the charts to indicate what data is in them, with
     * support for stacked compound features which are displayed with the colors
     * of the individual attributes within the sample.
     */
    ...
    /**
     * Update the focus element to interact with the sprint data, performing
     * a selection of the sprint upon hovering and filling the tooltip.
     * The focus overlay may also handle other clicks, for example on the area
     * of the legend, for toggling visibility of a feature.
     */
    ...
    /**
     * Handle a click on a legend area of a chart, which may toggle a feature to
     * be hidden or shown in the charts.
     */
    ...
    /**
     * Handle a click on a row of a legend, which may toggle a feature to be
     * hidden or shown in the charts.
     */
    ...
    /**
     * Update a row of a legend after being clicked to toggle the visiblity of
     * a feature.
     */
    ...
    /**
     * Update the axes of the charts to new metadata labels, domain ranges, and
     * positions for extra axes.
     */
    ...
    /**
     * Group features into lower and higher magnitudes.
     */
    ...
    /**
     * Fill the feature grouping elements with sprint data based on the way that
     * the chart output format type defines.
     */
    ...
    /**
     * Update the entire chart(s) based on the way that the chart output format
     * type defines.
     */
