...
export default class Format {
    constructor(locales, localization) {
        ...
        const formatDetailsUnit = (cell, v, key) => {
        ...
        this.details = {
            ...
            'domain_name': {
                order: 1,
                format: (cell, v, feature, sprints, source_ids) => {
    ...
    formatUnit(key, value, node, svg=false, adjust={}) {
        if (!this.defined(value)) {
            node.text(this.locales.message("no-value"));
            return;
        }
        ...
        node.text(vsprintf(unit, [this.formatNumber(value)]));
    }

    formatFeature(key, value, node, svg=false, adjust={}) {
        if (_.isObject(value)) {
        ...
        this.formatUnit(key, value, node, svg, adjust);
    }
