...
export default class Format {
    ...
    formatFixVersion(cell, v, data) {
        if (!this.defined(v)) {
            ...
            const issue_key = d3.select(cell.node().parentNode).select('td')
                .datum().key;
            const project_key = issue_key ? issue_key.split("-")[0] : data.project_name;
            cell.append('a')
                .attr('href', `${config.jira_url}/projects/${project_key}/versions/${v}`)
                .attr('target', '_blank')
                .text(fixversion);
        }
    }
