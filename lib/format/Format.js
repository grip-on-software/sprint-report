...
export default class Format {
    ...
    formatMetricTarget(key, value, node, date, metric) {
        ...
        if (!_.isEmpty(metric_target)) {
            ...
            node.classed('has-text-primary has-text-success has-text-warning has-text-danger', false);
            if (value === metric_target.perfect) {
                node.classed('has-text-primary', true)
                    .attr('title', this.locales.message('target-perfect'));
            }
            else if (metric_target.target === value ||
                metric_target.direction === (metric_target.target - value < 0)
            ) {
                node.classed('has-text-success', true)
                    .attr('title', this.locales.message('target-good',
                        [this.formatUnitText(key, metric_target.target)]
                    ));
            }
            else if (metric_target.low_target === value ||
                metric_target.direction === (metric_target.low_target - value < 0)
            ) {
                const interval =  [
                    this.formatUnitText(key, metric_target.low_target),
                    this.formatUnitText(key, metric_target.target)
                ];
                node.classed('has-text-warning', true)
                    .attr('title', this.locales.message('target-almost',
                        metric_target.direction ? interval : _.reverse(interval)
                    ));
            }
            else {
                node.classed('has-text-danger', true)
                    .attr('title', this.locales.message('target-bad',
                        [this.formatUnitText(key, metric_target.low_target)]
                    ));
            }
        }
    ...
    formatUnit(key, value, node, svg=false, adjust={}, date=null, metric=[]) {
        ...
        if (this.localization.value_icons[key] &&
            this.localization.value_icons[key][value]) {
            if (svg) {
                svg.select(`path.${adjust.class}`).remove();
                this.addIcon(svg,
                    this.localization.value_icons[key][value], adjust
                );
            }
            else {
                ...
                node.append('i')
                    .attr('title', text)
                    .attr('class',
                        this.localization.value_icons[key][value].join(' ')
                    );
            }
            return;
        }
