...
export default class Format {
    constructor(locales, localization) {
        ...
        const formatDetailsUnit = (cell, v, key, data) => {
        ...
        this.value_format = {
            icon: (key, value, node, unit, config) => {
    ...
    formatMetricTarget(key, value, unit, node, config) {
        ...
        if (config.metric) {
            ...
            if (idx >= 0) {
                metric_target = _.assign({}, metric_target, config.metric[idx]);
            }
        }
