...
export default class Format {
    constructor(locales, localization) {
        ...
        this.value_format = {
            icon: (key, value, node, unit, svg, adjust) => {
                if (this.localization.metadata[key].values.icons[value]) {
                    if (svg) {
                        svg.select(`path.${adjust.class}`).remove();
                        this.addIcon(svg,
                            this.localization.metadata[key].values.icons[value], adjust
                        );
                        return true;
                    }
                    else if (node !== null) {
                        ...
                        node.append('i')
                            ...
                            .attr('class',
                                this.localization.metadata[key].values.icons[value].join(' ')
                            );
                        return true;
                    }
                }
            ...
            duration: (key, value) => {
                var duration = {};
                const intervals = this.localization.metadata[key].values.intervals;
            ...
            fraction: (key, value, node, unit) => {
                const D = this.localization.metadata[key].values.denominator;
    ...
    formatUnitText(key, value, unit="%s", node=null, svg=false, adjust={}) {
        ...
        if (this.localization.metadata[key] &&
            this.localization.metadata[key].values &&
            this.value_format[this.localization.metadata[key].values.type]
        ) {
            const type = this.localization.metadata[key].values.type;
