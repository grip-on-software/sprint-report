    ...
    updateChart(state, chart, d, i, t) {
        ...
        this.updateFocus(chart, d, state, sprints, x, y, {
            ...
            focus: (focusHolder, i, pos) => {
                ...
                focusHolder.select("circle")
                    ...
                    .attr("cx", missing ? 0 : x(dates[i]))
                    .attr("cy", pos);
                focusHolder.select("line")
                    .attr("visibility", missing ? "hidden" : null)
                    .attr("x1", missing ? 0 : x(dates[i]))
                    .attr("x2", missing ? 0 : x(dates[i]))
                    .attr("y1", pos + 2);
            }
        });
        ...
        this.updateAxes(chart, state, d, x, y, {
            x: {
                ...
                label: this.locales.attribute("sprint_meta", "start_date")
            },
            y: {
                axis: d3.axisLeft(y),
        ...
        const line = d3.line()
            .defined(g => typeof g !== "undefined" && g !== null)
            .x((g, i) => x(dates[i]))
            .y(g => y(g))
