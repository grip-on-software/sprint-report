...
export default class LineChart extends Chart {
    ...
    updateChart(state, chart, d, i, t) {
        ...
        var x = d3.scaleTime()
            .range([0, width])
            .domain([_.min(dates), Math.max(_.max(futureDates), _.max(dates))]);
    ...
    updateFeatureText(dates, d, f, j, feature, config, t) {
        ...
        const lowest = _.isEmpty(d.errors[`${f.feature_key}_density`]) ? null :
            _.minBy(_.toArray(d.errors[`${f.feature_key}_density`]), findDensity);
        ...
        var texts = [];
        if (this.defined(lowest)) {
            texts = texts.concat(wrap(this.locales.message("prediction-error-probability", [
                this.localization.moment(config.futureDates[zero]).format('LL'),
                d3.format(".1~%")(lowest[zero])
            ])).lines);
        }
        if (this.defined(likelihood)) {
            texts = texts.concat(wrap(
                this.locales.message("prediction-error-margin", [
                    d3.format(".1~%")((1 - likelihood) * Math.log(Math.max(1, zero)))
                ])
            ).lines);
        }
