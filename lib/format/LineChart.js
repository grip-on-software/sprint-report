...
export default class LineChart extends Chart {
    ...
    format(data, state, resolve) {
        ...
        this.updateLegends(state, updateProjects, {
            ...
            color: sample => {
                sample.select("rect").style("fill",
                    (d, i) => this.scheme[i % this.scheme.length]
                );
                sample.select("use")
                    .attr("href",
                        (d, i) => `#${this.patterns[i % this.patterns.length].id}`
                    )
                    .attr("fill", (d, i) => this.scheme[i % this.scheme.length]);
            }
        });
    ...
    updateFeature(dates, f, j, feature, config, t) {
        ...
        points.merge(point)
            .attr("href", `#${this.patterns[j % this.patterns.length].id}`)
