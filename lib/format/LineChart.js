...
export default class LineChart extends Chart {
    ...
    updateChart(state, chart, d, i, t) {
        ...
        chart.select("g.features").selectAll("g.feature")
            .each((f, j, features) => {
                this.updateFeature(dates, d, f, j, d3.select(features[j]), {
    ...
    updateFeature(dates, d, f, j, feature, config, t) {
        const sprints = zipFeature(f.sprints);
        if (d.errors[f.feature_key]) {
            const zero = _.findLastIndex(sprints[0],
                (s, i) => i > config.futureIndex && s > 0
            ) - config.futureIndex;
            const numFuture = sprints[0].length - config.futureIndex;
            const p = (i, m=1) => `${config.x(dates[config.futureIndex+i])},${_.clamp(config.y(sprints[0][config.futureIndex+i] + (zero > 0 && i > zero ? zero : i) * m * d.errors[f.feature_key][i > numFuture / 3 && d.errors[f.feature_key][1] > d.errors[f.feature_key][0] ? 1 : 0]), 0, this.height)}`;
            const indexes = _.range(numFuture -1, -1, -1);
            // Draw a cone
            const cone = feature.selectAll("polygon")
                .data([d.errors[f.feature_key]]);
            cone.enter()
                .append("polygon")
                .attr("fill", "#fff")
