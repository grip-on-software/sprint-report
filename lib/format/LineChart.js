...
export default class LineChart extends Chart {
    ...
    updateFeatureCone(dates, d, f, j, feature, config, t) {
        ...
        const sprint = (i, k) => _.isObject(f.sprints[config.futureIndex+i]) ?
        ...
        const buildCone = (k) => {
            ...
            const error = (i) => (zero > 0 && i > zero ? zero : i) *
                e[i > numFuture / 3 && Math.abs(e[1]) > Math.abs(e[0]) ? 1 : 0];
            return _.map(_.range(0, numFuture), i => [sprint(i, k), error(i)]);
        };

        const area = d3.area()
            .curve(d3.curveMonotoneX)
            .x((g, i) => config.x(dates[config.futureIndex+i]))
            .y0((g, i) => _.clamp(config.y(g[1] < 0 ? g[0] : g[0] + g[1]), 0, this.height))
            .y1((g, i) => _.clamp(config.y(g[0] - Math.abs(g[1])), 0, this.height));

        // Draw a cone
        const cone = feature.selectAll("path")
    ...
    updateFeatureText(dates, d, f, j, feature, config, t) {
        ...
        const zero = d.errors[`${f.feature_key}_density`] ?
            _.min(_.map(d.errors[`${f.feature_key}_density`],
                density => _.findIndex(density, c => c === 1)
            )) : -1;
        const index = zero < d.errors.date.length / 3 ? 0 : 1;
        const nanMean = errors => _.meanBy(_.filter(errors,
            p => this.defined(p[index])
        ), p => Math.abs(p[index]));
        const mean = nanMean(d.errors[f.feature_key]);
        const points = zero === -1 ? mean :
            mean * Math.abs(zero - d.errors.date.length / 3);
        const texts = [];
        const likelihood = nanMean(d.errors[`${f.feature_key}_probability`]);
        if (zero !== -1 && this.defined(likelihood)) {
            texts.push(this.locales.message("prediction-error-probability", [
                this.localization.moment(d.errors.date[zero]).format('LL'),
                d3.format(".1~%")(likelihood)
            ]));
        }
        if (this.defined(points)) {
            texts.push(this.locales.message("prediction-error-margin", [
                this.value_format.fraction(f.feature_key, points, null, "%s"),
                this.locales.attribute("predictor-error", "points")
            ]));
        }
        ...
        text.enter()
            ...
            .merge(text)
            .attr('y', (d, i) => this.svgHeight + 0.8 * this.textHeight * i + 3)
            .text(d => d);
        ...
        if (d.errors[`${f.feature_key}_density`]) {
            ...
            const y = d3.scaleLinear()
                .rangeRound([this.height, 0])
                .domain([0, 1]);
            const date = _.map(d.errors.date,
                d => moment(d, "YYYY-MM-DD HH:mm:ss")
            );
            const line = d3.line()
                .defined(this.defined)
                .x((g, i) => config.x(date[i]))
                .y(g => y(g))
                .curve(d3.curveMonotoneX);
