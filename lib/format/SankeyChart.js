...
export default class SankeyChart extends Chart {
    ...
    convertSankey(diagram, sprints, data, details, state) {
        ...
        const cleanDuplicates = (keys, feature, earlierFeatures, sprint_id) => {
        ...
        const getLinks = (feature, earlierFeatures, s, i, otherSprintId) => {
            ...
            var remainingKeys = cleanDuplicates(sprint.key, feature,
            ...
            var link = _.map(details, (otherDetail, otherFeature) => {
                ...
                if (i > 0) {
                    return getLink(sprint, keys, i, feature, otherFeature);
                }
                return [];
            });
        ...
        const links = sprints.length === 1 ? [] : _.flattenDeep(_.map(sprints,
            (s, i) => {
                ...
                return _.map(Array.from(features), feature => {
                    const link = getLinks(feature, earlierFeatures,
                        s, i, otherSprintId
                    );
