...
export default class SankeyChart extends Chart {
    ...
    updateChart(state, chart, d, i, t) {
        ...
        axios.all(_.map(Array.from(extra),
            ...
            this.updateSankey(chart, diagram, d, data, details, state, t);
        });
    }

    updateSankey(chart, diagram, chartData, data, details, state, t) {
        ...
        const { nodes, links } = this.convertSankey(diagram, chartData, data,
            details, state
        );
        ...
        newNodes
            .attr("transform", d => `translate(${d.x0}, ${height - (d.y1 - d.y0)})`)
            .merge(updateNodes)
            .transition(t)
        ...
        newNodes.append("title").each((d, i, nodes) => {
            ...
            node.append("text").text(`${this.formatSprint(
                chartData.sprints[d.sprint_id], node, state.sprint_meta, "main"
        ...
        newLinks.append("title")
            ...
            .each((d, i, nodes) => {
                const node = d3.select(nodes[i]);
                node.append("text").text(`${this.formatSprint(
                    chartData.sprints[d.source.sprint_id],
                    node, state.sprint_meta, "main"
                )} \u21d2 ${this.formatSprint(chartData.sprints[d.target.sprint_id],
                    node, state.sprint_meta, "main"
                )}\n${this.locales.retrieve(this.localization.descriptions,
    ...
    convertSankey(diagram, d, data, details, state) {
        ...
        const getLink = (sprint, keys, d, i, feature, otherFeature) => {
            const value = _.sum(_.map(sprint.key,
                (k, j) => keys.includes(k) ?
                    getPoints(sprint, j, d.sprints[i][feature]) : 0
            ));
            if (value === 0) {
                return null;
            }
        ...
        const links = _.flattenDeep(_.map(d.sprints, (s, i) => {
            if (d.sprints.length === 1) {
                return [];
            }

            const otherSprintId = d.sprints[i === 0 ? i + 1 : i - 1].sprint_id;
            ...
            return _.map(Array.from(features), feature => {
                ...
                var link = _.map(details, (otherDetail, otherFeature) => {
                    ...
                    if (i > 0) {
                        return getLink(sprint, keys, d, i, feature,
                            otherFeature
                        ) || [];
                    }
                    return [];
                ...
                if (!remainingKeys.isEmpty()) {
                    const extraLink = getLink(sprint, remainingKeys, d, i,
                        feature, "_extra"
                    );
                    if (extraLink !== null) {
