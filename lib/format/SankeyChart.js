...
export default class SankeyChart extends Chart {
    ...
    updateChart(state, chart, d, i, t) {
        ...
        const diagram = sankey()
            .nodeId(d => d.key)
            .nodeAlign((node, n) => n < d.sprints.length ? node.depth :
        ...
        axios.all(_.map(Array.from(extra),
            ...
            this.updateSankey(chart, diagram, d.sprints, data, details, state, t);
        });
    }
            ...
            .call(d3.drag().on("start", function(node) {
                ...
                chart.select("g.nodes")
                    ...
                    .filter(d => d !== node && d.x0 === node.x0 &&
                        !d.invisible && nodeOverlaps(d, node)
                    )
                    .each((d, i, nodes) => {
    ...
    getLink(sprints, sprint, keys, i, feature, otherFeature) {
        if (keys.isEmpty()) {
            return [];
        }
    ...
    cleanDuplicates(details, features, keys, feature, earlierFeatures, sprint_id) {
        const candidates = Array.from(feature.startsWith('backlog_') ?
            features.delete(feature) :
            earlierFeatures.filter(feature => !feature.startsWith('backlog_'))
        );
        return _.reduce(candidates,
            (accumulator, concurrentFeature) => {
                const sprint = this.getSprintDetails(
                    details[concurrentFeature], sprint_id
                );
                if (!sprint.key) {
                    return accumulator;
                }
    ...
    convertSankey(diagram, sprints, data, details, state) {
        const nodes = this.getNodes(sprints, data, details);
        ...
        const getLinks = (feature, earlierFeatures, s, i, otherSprintId) => {
            ...
            var remainingKeys = this.cleanDuplicates(details, features, sprint.key, feature,
                earlierFeatures, s.sprint_id
            );
            var link = _.map(details, (otherDetail, otherFeature) => {
                ...
                if (i > 0) {
                    return this.getLink(sprints, sprint, keys, i, feature,
                        otherFeature
                    );
                }
                return [];
            ...
            if (!remainingKeys.isEmpty()) {
                const extraLink = this.getLink(sprints, sprint, remainingKeys,
                    i, feature, "_extra"
                );
        ...
        const links = sprints.length === 1 ? [] : _.flattenDeep(_.map(sprints,
            (s, i) => getSprintLinks(s, i)
        ));
