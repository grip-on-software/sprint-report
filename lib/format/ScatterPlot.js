/**
 * Scatter plot format for the sprint report.
 *
...
/**
 * Scatter plot chart output format.
 */
export default class ScatterPlot extends Chart {
    initialize() {
        super.initialize();

        // Indicate that we only have one chart for all projects, instead of
        // multiple charts with one for each.
        const charts = this.content.selectAll('svg').data([0]);

        // Create the charts, with grouping elements for sprint feature data,
        // clusters and regression lines.
        ...
        // Replace the focus overlay with a brush overlay
        overlay.remove();
        newProjects.append("g")
            .classed("brush", true)
            .datum([]);
    ...
    /**
     * Create a brush element which can perform a zoom by selecting an area or
     * double clicking.
     */
    ...
    /**
     * Handle the end of a brush operation, either zooming in based on a brush
     * selection, pinning the focus element or zooming out.
     */
    endBrush(brush, state) {
        // Don't do anything if chart is not set up
        if (this.x === null) {
            return;
        }
        ...
        if (selection) {
            ...
            if (this.brushFocus(this.focus.pinTooltip)) {
                // Handle a click on the overlay to pin the tooltip
            ...
            if (!this.idleTimeout) {
                // Handle a click to wait for a second click
                // Poor man's double click emulation
                this.idleTimeout = setTimeout(() => {
                    this.idleTimeout = null;
                }, this.brushDelay);
                return;
            }
            // Handle a double click to zoom out to the default domain
            x.domain(this.x.domain);
            y.domain(this.y.domain);
        }
        // If we get here, then the zoom of the chart is adjusted based on the
        // new domain
        this.zoom(state);
    }

    /**
     * Determine if a focus element action should be taken based on an event
     * triggered on the brush overlay. Actions could be moving or pinning the
     * tooltip.
     */
    brushFocus(action) {
    ...
    /**
     * Check whether the coordinates are within the current visible area of
     * a potentially zoomed-in chart.
     */
    coordsInDomain(x, y) {
    ...
    /**
     * Determine if a sprint defined by the two primary features is visible
     * within the current domain of the chart.
     */
    pointInDomain(point) {
    ...
    /**
     * Check if a sprint has two primary features that can be plotted.
     */
    isDefined(point) {
    ...
    /**
     * Perform a zoom to the new domain of the chart.
     */
    zoom(state, t=null) {
        this.zooming = true;
        const chart = this.content.select('svg');

        // Hide the focus elements until another action displays it again
        ...
        // Update positions of circles and clusters
        ...
        // Update regression lines and labels
    ...
    /**
     * Determine new cluster positions and sizes based on sprints that are
     * nearby each other for the two primary features and the chart scale.
     */
    updateClusters(circles) {
        ...
        this.groups = _.reduce(circles.nodes(), (accumulator, node) => {
            ...
            if (group.size() > 1) {
                ...
                let other = findCluster(cluster, accumulator);
                if (other !== null) {
                    let newOther = findCluster(other, accumulator);
                    while (newOther !== null) {
    ...
    /**
     * Determine sprints to display as points in the scatter plot. If there are
     * not enough selected features (need two or more), then no points are
     * displayed.
     */
    ...
    /**
     * Create or update the points that indicate the two primary features of
     * sprints within the scatter plot.
     */
    ...
    /**
     * Creatr or update regression lines by fitting the points in the scatter
     * plot and indicating the regression fit metric (r-squared) with the label.
     */
    ...
    /**
     * Find the distance between a focus position and the given sprint point
     * and check if it is nearer than the clostest distance found so far.
     */
    ...
    updateChart(state, chart, d, i, t) {
        ...
        // Fill the sprint points and regression lines
    ...
    /**
     * Create clickable options within a pinned focus tooltip that makes use of
     * the context of the nearby sprint point for cycling through the points of
     * a cluster, zooming, and temporarily removing the point to see what this
     * does to the clusters.
     */
