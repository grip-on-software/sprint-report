...
export default class ScatterPlot extends Chart {
        ...
        this.updateLegends(state, chart, {
            ...
            sampleExtra: (d, i, legend) => legend
                ...
                .attr("stroke", d.axis ? "#000" : this.scheme[
                    (state.projects.selected.size + i - 1) % this.scheme.length
                ]),
    ...
    updateRegression(state, chart, points) {
        ...
        regressions.select("path")
            .attr("stroke", (d, i) => this.scheme[
                (state.projects.selected.size + i) % this.scheme.length
            ])
    ...
    comparePointsDistance(minimal, point, index, features, x, y) {
        if (!this.pointInDomain(point)) {
            return minimal;
        }
        const distance =
            Math.abs(this.x.scale(point[features[0]]) - x) +
            Math.abs(this.y.scale(point[features[1]]) - y);
