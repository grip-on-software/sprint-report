...
export default class Table extends Format {
    ...
    format(data, state, resolve) {
        ...
        features.selectAll('table.details').remove();
        ...
        updateFeatures.each((d, i, nodes) => {
            d3.select(nodes[i])
                ...
                .each((v, j, sprints) => this.formatFeature(d.feature_key, v,
                    d3.select(sprints[j]), false, {}, d.start_date[j], []
                ));
        });
    ...
    addSourceIcon(item, data, state) {
        ...
        item.append('a')
            ...
            .attr('aria-label', d => this.locales.message('source-label'))
            .attr('data-tooltip', d => this.locales.message('source-tooltip',
        ...
        item.append('a')
            ...
            .attr('aria-label', this.locales.message('details-expand-title'))
            ...
            .on('click', (d, i, nodes) => {
                ...
                const sprint_ids = _.map(data.sprints,
    ...
    buildDetails(feature_key, icon, sprints, sprint_ids, details, data) {
        const tables = sprints.selectAll('table.details')
            .data((s, j) => {
                const sprint = this.getSprintDetails(details, sprint_ids[j]);
                return [{
        ...
        if (!tables.empty()) {
            ...
            icon.attr('aria-label', expand)
                .attr('data-tooltip', expand)
                .select('i')
        ...
        this.updateDetails(feature_key, sprints, tables, orders, data);
        sprints.node().scrollIntoView();
    ...
    updateDetails(feature, sprints, tables, sortOrders, data) {
        const table = tables.enter().append('table')
            .classed('details table is-narrow has-text-weight-normal has-background-white', true);
        ...
        newHeader.merge(header.select('a'))
            ...
            .attr('title', k => this.locales.message(`details-sort-${sortOrders[k] && sortOrders[k].dir === 'asc' ? 'descending' : 'ascending'}`,
                [this.locales.attribute("details", k)]
            ))
            .attr('class', k => {
                const parts = ['sort'];
                if (sortOrders[k]) {
        ...
        cell.merge(newCell).each((z, i, cells) => {
            ...
            if (!details || !details.format) {
                ...
                details.format(cell, z[1], feature, data.source_ids,
                    data.sprints[z[2]] ? data.sprints[z[2]].start_date : null,
                    data.metric_targets && data.metric_targets[feature] ?
