...
export default class Table extends Format {
        ...
        item.append('a')
            ...
            .on('click', (d, i, nodes) => {
                ...
                if (state.features.default.includes(d.feature_key)) {
                    this.buildDetails(d.feature_key, icon, sprint_ids,
                    ...
                    axios.get(`data/${data.project_name}/details.${d.feature_key}.json`).then((details) => {
                        this.buildDetails(d.feature_key, icon,
    ...
    buildDetails(feature_key, icon, sprint_ids, details, data) {
        const sprints = d3.select(icon.node().parentNode.parentNode)
            .selectAll('td.sprint');
