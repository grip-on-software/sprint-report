...
export default class Table extends Format {
    ...
    requestConfig(state) {
        var config = super.requestConfig(state);
        config.details = true;
        return config;
    }
        ...
        item.append('a')
            ...
            .on('click', (d, i, nodes) => {
                ...
                if (state.features.default.includes(d.feature_key)) {
                    this.buildDetails(d.feature_key, icon, sprints, sprint_ids,
                        data.details[d.feature_key], data.source_ids
                    );
                }
                else {
                    const detailsSpinner = new spinner({
                        width: sprints.node().clientWidth,
                        height: 0.8 * sprints.node().clientHeight,
                        startAngle: 220,
                        container: sprints.node(),
    ...
    buildDetails(feature_key, icon, sprints, sprint_ids, details, source_ids) {
        const tables = sprints.selectAll('table.details')
            .data((s, j) => {
                const sprint_id = _.isArray(sprint_ids[j]) ?
                    _.find(sprint_ids[j], id => !!details[id]) : sprint_ids[j];
                const sprint = details[sprint_id];
                return [{
        ...
        if (!tables.empty()) {
            tables.transition()
                ...
                .on("end", () => {
                    tables.remove();
                    sprints.node().scrollIntoView();
                });
            icon.attr('data-tooltip',
                this.locales.message('details-expand-title')
            );
            icon.select('i')
        ...
        this.updateDetails(feature_key, sprints, tables, orders, source_ids);
        sprints.node().scrollIntoView();

        icon.attr('data-tooltip',
            this.locales.message('details-collapse-title')
        );
        icon.select('i')
    ...
    updateDetails(feature, sprints, tables, sortOrders, source_ids) {
        const table = tables.enter().append('table')
            .classed('details table is-narrow', true);

        const transition = table.style("opacity", 0)
            .transition()
            .style("opacity", 1);
        ...
        newHeader.merge(header.select('a'))
            .on("click", (k) => {
                ...
                const sortTables = sprints.selectAll('table.details')
                    .data((s, j, table) => [
                        d3.select(table[j]).selectAll('table.details').datum()
                    ]);
                this.updateDetails(feature, sprints, sortTables, orders,
                    source_ids
                );
            })
            .select('i')
        ...
        const detail = tables.merge(table).selectAll('tr.detail')
            .data(t => {
                ...
                const iteratees = _.map(sortable,
                    k => (row) => this.details[k] && this.details[k].sort ?
                        this.details[k].sort(row[k]) : row[k] 
                );
                const orders = _.map(sortable, k => sortOrders[k].dir);
                return _.orderBy(t.rows, iteratees, orders);
            })
            .order();
        ...
        cell.merge(newCell).each((z, i, cells) => {
            ...
            if (!details || !details.format) {
                ...
                details.format(cell, z[1], feature, sprints, source_ids);
            }
        });
    }
}
