                ...
                const detail = table.selectAll('tr.detail')
                    .data(t => {
                        const iteratees = _.map(
                            _.filter(_.keys(t[0]),
                                k => this.details[k] && this.details[k].sort
                            ),
                    ...
                    .classed('detail', true);
                detail.selectAll('td')
                    .data(z => _.sortBy(_.toPairs(z), p => this.details[p[0]] ?
                        this.details[p[0]].order : Infinity
                    ))
                    .enter()
                    .append('td')
                    .each((z, i, cells) => {
                        ...
                        if (!details || !details.format) {
                            ...
                            details.format(cell, z[1], d.feature_key);
                        }
                    });
