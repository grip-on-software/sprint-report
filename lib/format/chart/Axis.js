...
export default class Axis {
    ...
    update(t, x, y, locales, localization) {
        ...
        var text = this.config.label.text;
        if (this.config.label.features) {
            ...
            else if (this.config.label.features.map(
                f => localization.metadata[f].measurement ?
                    localization.metadata[f].measurement.unit : undefined
            ).size === 1) {
                text = locales.attribute("units",
                    localization.metadata[this.config.label.features.first()]
                        .measurement.unit
                );
            }
        }
