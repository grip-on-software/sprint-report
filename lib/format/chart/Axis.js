    ...
    update(t, x, y, locales, localization) {
        ...
        if (this.config.label.features) {
            if (this.config.label.features.size === 1) {
                ...
                this.config.label.features.map(f =>
                    localization.metadata.measurement[f] ?
                    localization.metadata.measurement[f].unit : undefined
                ).size === 1
            ) {
                text = locales.attribute("units",
                    localization.metadata.measurement[this.config.label.features.first()].unit
                );
            }
        }
