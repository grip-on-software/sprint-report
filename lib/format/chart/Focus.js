...
export default class Focus {
    ...
    updateTooltipOptions(tooltip, metadata, index) {
        ...
        options.enter().append("g")
            ...
            .merge(options).order()
            .each((d, i, nodes) => {
                this.updateOption(tooltip, metadata, text, d, i, nodes);
            });
    }

    updateOption(tooltip, metadata, text, d, i, nodes) {
    ...
    clickTooltip(tooltip, target, position, current) {
        ...
        const item = tooltip.selectAll("tspan.meta, tspan.option")
            .filter((d, j, nodes) => {
                return this.findClickedItem(target, position, meta, rects,
                    d, j, nodes
                );
            });
        if (item.empty()) {
    ...
    findClickedItem(target, position, meta, rects, d, j, nodes) {
